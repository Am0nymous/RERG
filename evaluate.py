...
def main():
    ...
    model = FaBERT(config)
    trained_path = '/share/home/zhaogz/codes/copy/Third/Tabfact/rl_large/output/base/save_step_96768_acc_0.8320717908700741/model.bin'
    model.load_state_dict(torch.load(trained_path)['net'])
    agent = FC_net()
    agent.load_state_dict(torch.load(trained_path)['agent'])
    global_step = -1
    best_acc = -1
    if args.do_train:
        data_type = "train"
