...
def main():
    ...
    if args.do_train:
        ...
        train_dataloder, num_train_examples = prepare_data(args, data_type)
        optimizer = AdamW(model.parameters(), lr=args.lr, eps=1e-8, weight_decay=1e-4)
        t_total = len(train_dataloder) * args.num_train_epochs
        pg_optimizer = AdamW(agent.parameters(), lr=0.0001, weight_decay=0.01)
        warm_up_steps = 0.1 * t_total
        scheduler = get_scheduler(optimizer,args.scheduler, warm_up_steps, t_total)
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloder, desc="Iteration")):
                batch = tuple(t.to(device) for t in batch)
                input_ids, attention_mask, segment_ids, column_ids, row_ids, prev_label_ids, column_ranks, inv_column_ranks, \
                numeric_relations, label_id, position_ids, adj_matrix, graph_mask, bert_words, bert_maps, word_masks, start_pos, end_pos = batch
                ...
                logit, action_loss, entropys, rp_loss = model.forward(input_ids, attention_mask, token_type_ids,
                                                                      ...
                                                                      bert_words, bert_maps, word_masks, start_pos,
                                                                      end_pos, flag, agent)

                loss = loss_fn(logit, label_id.squeeze(1))
                ...
                logit, _, _, _ = model.forward(input_ids, attention_mask, token_type_ids,
                                               position_ids, adj_matrix, graph_mask.float(), bert_words, bert_maps,
                                               word_masks, start_pos, end_pos, flag, agent)
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    preds = torch.argmax(logit, -1) == label_id.squeeze(1)
                    ...
                    logging.info(
                        "lr: {:.7f} loss: {:.5f} loss_rl: {:.5f} reward: {:.5f} acc: {:.5f}".format(
                            learning_rate_scalar, loss, loss_rl, reward, acc))
                if (step + 1) % 50 == 0:
                    torch.cuda.empty_cache()
                if epoch >= 4:
                    alpha = 10
                    ...
                    alpha = 5
                if (step + 1) % (len(train_dataloder) // alpha) == 0:
                    logging.info("This period loss :{} ...".format(tr_loss / (len(train_dataloder) // alpha)))
                    logging.info("This period reward :{} ...".format(tr_reward / (len(train_dataloder) // alpha)))
                    writer.add_scalar('train/period_loss', tr_loss / (len(train_dataloder) // alpha), global_step)
                    writer.add_scalar('train/period_reward', tr_reward / (len(train_dataloder) // alpha), global_step)
                    torch.cuda.empty_cache()
                    ...
                    if dev_result["acc"] > best_acc:
                        best_acc = dev_result["acc"]
                        model_to_save = model.module if hasattr(model, 'module') else model  # Only save the model it-self
                        output_dir = os.path.join(args.output_dir, 'save_step_{}_acc_{}'.format(global_step, best_acc))
                        if not os.path.exists(output_dir):
                            os.makedirs(output_dir)
                        if epoch >= 2:
                            model_dicts = {"net": model_to_save.state_dict(), "agent": agent.state_dict()}
                            torch.save(model_dicts, os.path.join(output_dir, "model.bin"))
                        if epoch >= 2:
                            logging.info("Testing the test data ....")
                            test_result = evaluate(args, model_to_save, device, "test", agent)

                            logging.info("Testing the simple_test data ....")
                            simple_test_result = evaluate(args, model_to_save, device, "simple_test", agent)

                            logging.info("Testing the small_test data ....")
                            small_test_result = evaluate(args, model_to_save, device, "small_test", agent)

                            logging.info("Testing the complex_test data ...")
                            complex_test_result = evaluate(args, model_to_save, device, "complex_test", agent)
