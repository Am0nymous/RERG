...
class TapasTokenizer:
  ...
  def __init__(self, vocab_file):
    ...
    #self._wp_tokenizer = RobertaTokenizer.from_pretrained('roberta-base', do_lower_case=True)
...
class ToTensorflowExampleBase:
  ...
  def _add_numeric_relations_graph(self, question,
                             column_ids, row_ids,
                             table,
                             features):
    """Adds numeric relation emebeddings to 'features'.
    ...
    indexs = []
    numeric_relations = [0 for _ in column_ids]
    text = ('[CLS] ' +  question.original_text + " [SEP]").split(" ")
    t2w = {}
    index = 0
    for i, t in enumerate(question.text):
      if t == ' ':
        index += 1
      t2w[i] = index
    ...
    if question is not None and table is not None:
      for numeric_value_span in question.annotations.spans:
        for value in numeric_value_span.values:
          cell_indices_to_relations = collections.defaultdict(set)
          index_ = []
          for i in range(numeric_value_span.begin_index, numeric_value_span.end_index):
              try:
                  index_.append(t2w[i] + 1)
              except KeyError:
                  print(question.text)
                  print(question.original_text)
                  print(question.text[numeric_value_span.begin_index:numeric_value_span.end_index])
                  print(t2w)
                  pass
          for column_index in range(len(table.columns)):
            table_numeric_values = self._get_column_values(table, column_index)
            ...
            if sort_key_fn is None:
              continue
            for row_index, cell_value in table_numeric_values.items():
              relation = number_annotation_utils.get_numeric_relation(value, cell_value, sort_key_fn)
              if relation is not None:
                cell_indices_to_relations[column_index, row_index].add(relation)
          for (column_index, row_index), relations in cell_indices_to_relations.items():
            ...
            for relation in relations:
              assert relation.value >= constants.Relation.EQ.value
              relation_set_index = relation.value
            for cell_token_index in _get_cell_token_indexes(column_ids, row_ids, column_index, row_index):
              numeric_relations[cell_token_index] = relation_set_index
          number_relations.append(numeric_relations)
          indexs.append(index_)
    return (number_relations, indexs)
  ...
  def _pad_to_seq_length(self, inputs):
    ...
    while len(inputs) < self._max_seq_length:
      inputs.append(0)
  def _pad_to_seq_length_input(self, inputs):
    while len(inputs) > self._max_seq_length:
      inputs.pop()
    while len(inputs) < self._max_seq_length:
      inputs.append(1)
  ...
  def _to_features(
    ...
    relations_graph = self._add_numeric_relations_graph(question, token_ids_dict['column_ids'],
                                token_ids_dict['row_ids'], table, features)
    ...
    return features, relations_graph
...
class ToTrimmedTensorflowExample(ToTensorflowExampleBase):
  ...
  def _to_trimmed_features(
    ...
    features, graph= self._to_features(
        serialized_example.tokens, feature_dict, table=table, question=question)
    return serialized_example, features, graph
...
class ToClassifierTensorflowExample(ToTrimmedTensorflowExample):
  ...
  def convert(self, interaction,
    ...
    serialized_example, features, graph = self._to_trimmed_features(
        question=question,
        table=table,
    ...
    return tf.train.Example(features=tf.train.Features(feature=features)), graph
